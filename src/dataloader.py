...
class QueryPassageDataset(Dataset): ## TODO: Add deep_ct
    def __init__(self, 
        filepath, biggraph = False, deepct = False):
        """
        Args: 
    ...
    def __getitem__(self, idx):
        sample = {  'query_sentence': self.df.iloc[idx].query_text,
                    'retrieval_sentence': self.df.iloc[idx].abstract,
                    }
        if self.biggraph:
            sample['biggraph'] = self.df.iloc[idx].biggraph_embedding
        if self.deepct:
            sample['deepct'] = np.nan_to_num(self.df.iloc[idx].deepct_weights)
        return sample
...
if __name__=='__main__':
    ...
    # sample = data[0]
    # pdb.set_trace()
    data = QueryPassageDataset('/home/parham/entity-ranking/data/processed/non_triplet_folds/test_set_fold_0.parquet.gzip',biggraph = True, deepct = True)
    sample = data[0]
